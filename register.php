...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit'])) {
    ...
    move_uploaded_file($tempnam, $imagenam);
    if($password==$confirm_password){

    $qry = "INSERT IGNORE INTO `users` (`email`, `name`, `pnumber` , `image`) VALUES ('$email', '$fullname', '$phn' , '$imagenam');";
    $run = mysqli_query($dbcon,$qry);

    if($run==true){

        $psqry = "INSERT IGNORE INTO `login` (`email`, `password`, `u_id`) VALUES ('$email', '$password',LAST_INSERT_ID() )";
        ...
        ?>  <script>
            alert('Password Mismatch password and confirm password must be same');
            window.open('index.php','_self');
            </script>
        <?php
    }
                ...
                <div class="col-md-12">
                    ...
                    <form action="" method="post">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="ph" pattern="[7-9]{1}[0-9]{9}" class="form-control" required>
                        </div>    
                        <div class="form-group">
                            <label>Email Address</label>
                            <input type="email" name="email" pattern="[A-Za-z]+@gmail.com" class="form-control" required />
                        </div>    
                        <div class="form-group">
                            <!-- <label>Image</label>
